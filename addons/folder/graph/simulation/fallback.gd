...
func simulate() -> void:
	...
	for index in range(graph.EV.size()):
		...
		if absf(uv_velocity.x) < EPSILON: uv_velocity.x = 0.0
		if absf(uv_velocity.y) < EPSILON: uv_velocity.y = 0.0
		if absf(uv_velocity.z) < EPSILON: uv_velocity.z = 0.0
		...
		if absf(length - length0) < EPSILON: f_axial = Vector3.ZERO
		...
		if absf(h1) < EPSILON: nh1 = Vector3.ZERO
		if absf(h2) < EPSILON: nh2 = Vector3.ZERO
		...
		var c1 := maxf(uc1 + vc1, EPSILON)
		var c2 := maxf(uc2 + vc2, EPSILON)
		...
		if absf(a - a0 * fold_percent) < EPSILON: f_crease = 0.0
