...
static func validate_VC_sizes(graph: FoldGraph) -> bool:
	if graph.VC.is_empty(): return true
...
static func validate_EA_with_EFA(graph: FoldGraph) -> bool:
	if graph.EA.is_empty(): return true
	if graph.EFA.is_empty(): return true
...
static func validate_reflexive(array1: Array, array2: Array) -> bool:
	if array1.is_empty(): return true
	if array2.is_empty(): return true
...
static func validate_VV_VE_winding(graph: FoldGraph) -> bool:
	if graph.VV.is_empty(): return true
	if graph.VE.is_empty(): return true
	if graph.EV.is_empty(): return true
...
static func validate_VV_VF_winding(graph: FoldGraph) -> bool:
	if graph.VV.is_empty(): return true
	if graph.VF.is_empty(): return true
	if graph.FV.is_empty(): return true
...
static func validate_VE_VF_winding(graph: FoldGraph) -> bool:
	if graph.VE.is_empty(): return true
	if graph.VF.is_empty(): return true
	if graph.FE.is_empty(): return true
...
static func validate_FV_FF_winding(graph: FoldGraph) -> bool:
	if graph.FV.is_empty(): return true
	if graph.FF.is_empty(): return true
...
static func validate_FE_FF_winding(graph: FoldGraph) -> bool:
	if graph.FE.is_empty(): return true
	if graph.FF.is_empty(): return true
	if graph.EF.is_empty(): return true
...
static func _validate_frame_metadata(frame: FoldFrame) -> bool:
	...
	for index in range(FN.size()):
		if FN[index].size() != graph.FV[index].size():
			return false
	for index in range(FT.size()):
		if FT[index].size() != graph.FV[index].size():
			return false

	for vp in VP:
		if not vp.is_valid_html_color():
			return false
	for nc in NC:
		if nc.size() != 3:
			return false
